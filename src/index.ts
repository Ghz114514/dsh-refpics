...
import { Config, REFPICS_SETTINGS_NAMESPACE, keyFor, resolveCallRequest, resolveConfig, type ResolvedConfig } from './config.ts'
import {
  DEFAULT_COUNT,
  MAX_COUNT,
  ORIENTATIONS,
...
import { searchProvider, clampCount } from './providers.ts'
import { registerRefpicsRoutes } from './routes.ts'
...
const TOOL_HEAD =
  'Search stock design platforms for reference images matching a natural-language description and '
...
export function searchRefsCallView(args: SearchRefsArgs): GenericCallView {
  return {
    card: 'generic',
    title: '搜参考图',
    kind: 'search',
...
export function createSearchCache(maxEntries = 64, ttlMs = 10 * 60_000): SearchCache {
  const entries = new Map<string, { expires: number; value: RefPicsOutcome }>()
  return {
    get(key) {
...
export function cacheKey(provider: string, query: string, page: number, count: number, orientation: Orientation): string {
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: SearchRefsArgs, exec): Promise<RefPicsOutcome> {
      const active = spec()
